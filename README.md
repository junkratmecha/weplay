...
- 実装部分<br>
コミュニティへの加入申請→承認という流れを作るため、中間テーブル(Requestモデル・Belongingモデルの二つを作成。<br>
グループメッセージをAjaxにより非同期処理化(Jquery)
...
## 機能
* ユーザー機能(Device gem、ゲストログイン、セッション管理)
* コミュニティ機能(管理者ユーザー、中間テーブルによる参加申請→許可)
* グループメッセージ機能、非同期処理(Ajax)
